...
speeds = {'car': 2.45, 'bus': 1.8, 'truck': 1.8, 'bike': 2.5, 'emergency': 3.0}  # Average speeds of vehicles including emergency vehicle
...
plt.title('Average Waiting Time of ML Simulation')
...
class Environment:
    def __init__(self):
        self.currentGreen = 0  # Initialize the current green signal
        self.vehicles = {'right': [[] for _ in range(3)], 'down': [[] for _ in range(3)], 'left': [[] for _ in range(3)], 'up': [[] for _ in range(3)]}
        self.simulation = []  # Initialize the simulation list

    def get_state(self):
        vehicle_counts = [0, 0, 0, 0]  # Assuming four directions: right, down, left, up
        discretization_factor = 10  # Adjust this value as needed

        # Count the vehicles that have not crossed the stop line in each direction
        for direction_number, direction in enumerate(['right', 'down', 'left', 'up']):
            for lane in range(3):  # Assuming three lanes
                vehicle_counts[direction_number] += len([v for v in self.vehicles[direction][lane] if not v.crossed])

        # Discretize the vehicle counts
        vehicle_counts = [round(count / discretization_factor) for count in vehicle_counts]

        # The current green signal is already tracked by `currentGreen`
        current_green_signal = self.currentGreen

        # Combine these into a state
        state = (tuple(vehicle_counts), current_green_signal)
        return state

    def perform_action(self, state, action):
        # Calculate the total waiting time before the action
        total_waiting_time_before = sum(vehicle.waiting_time for vehicle in self.simulation)

        # Change the green signal to the action direction
        self.currentGreen = action
    
        # Calculate the total waiting time after the action
        total_waiting_time_after = sum(vehicle.waiting_time for vehicle in self.simulation)

        # Calculate the reward as the negative of the change in total waiting time
        reward = -(total_waiting_time_after - total_waiting_time_before)

        # Get the new state
        new_state = self.get_state()

        # Return the new state and reward
        return new_state, reward  # Swap the order of new_state and reward

def run_q_learning(alpha, gamma, epsilon, num_episodes, num_steps_per_episode, num_states, num_actions, get_state, perform_action):
    ...
    for episode in range(num_episodes):
        ...
        for step in range(num_steps_per_episode):
            ...
            if np.random.uniform(0, 1) < epsilon:
                action = np.random.choice(num_actions)  # Explore action space
            else:
                action = np.argmax(Q_table[state])  # Exploit learned values

            # Perform the action and get the new_state and reward
    ...
    # After learning, use the Q-table to make decisions
    state = get_state()
    while True:  # Continue indefinitely
        action = np.argmax(Q_table[state])  # Choose the action with the highest Q-value
        new_state, reward = perform_action(state, action)  # Perform the action
...
class Main:
    ...
    num_steps_per_episode = 100  # Number of steps per episode
    num_states = 100  # Number of states
    num_actions = 4  # Number of actions

    # Initialize your environment and Q-table
    environment = Environment()
    Q_table = np.zeros((num_states, num_actions))
    # Run Q-learning
    for episode in range(num_episodes):
        state = environment.get_state()  # Call get_state method
        state = np.ravel_multi_index(state[0], (num_states, num_states, num_states, num_states)) + state[1]

        for step in range(num_steps_per_episode):
            action = np.argmax(Q_table[state])  # Choose action based on Q-table

            # Perform the action and get the new state and reward
            new_state, reward = environment.perform_action(state, action)  # Call perform_action method
            new_state = np.ravel_multi_index(new_state[0], (num_states, num_states, num_states, num_states)) + new_state[1]
        ...
        # Compute the average waiting time after each episode
        # average_waiting_time = environment.compute_rl_average_waiting_time()  # Call compute_rl_average_waiting_time method
    ...
    while True:
        ...
        # Update the game state and redraw the game screen on each iteration of the loop
        def update_state(self):
            # Update the game state based on the current Q-table
        ...
        if frame_counter % 100 == 0:
            ...
            if total_vehicles_waited > 0:
                ...
                # Update the plot for existing average waiting time
                ...
                # Add the RL average waiting time to the plot
